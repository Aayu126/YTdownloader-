...
app.get('/api/download', async (req, res) => {
    try {
        ...
        // --- Logic to decide download method ---
        if (format.hasAudio && format.hasVideo) {
            // ✅ FAST DOWNLOAD: Stream directly if format has both audio and video
            console.log(`Performing fast download for video: ${title}`);
            ytdl(videoUrl, { quality: itag }).pipe(res);
        } else {
            // 🐌 SLOW DOWNLOAD: Use ffmpeg to merge separate streams
            console.log(`Performing slow download (ffmpeg merge) for video: ${title}`);
